...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        #python3 ok -q 09 only 8 test case passing 
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if hasattr(insect, 'imposter') and insect.imposter == False:
                return
            # END Problem 13
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return other.container == False and self.container and self.ant == None
        # END Problem 9
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        #if insect not watersafe kill it by reducing armor 
...
class LongThrower(ThrowerAnt):
    ...
    # BEGIN Problem 4
    def nearest_bee(self, hive):
        search_location = self.place.entrance.entrance.entrance.entrance.entrance
        while (search_location != hive):
            if search_location.bees == []:
                search_location = search_location.entrance
            else:
...
class ShortThrower(ThrowerAnt):
    ...
    def  nearest_bee(self, hive):
        search_location = self.place
        count = 0
        while (search_location != hive and count < self.max_range):
            if search_location.bees == []:
                search_location = search_location.entrance
                count += 1
            else:
...
class WallAnt(Ant):
    """A WallAnt has a large amount of armor"""
    name = 'Wall'
    food_cost = 4
    implemented = True 
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if self.ant != None:
            self.ant.action(colony)
        bee_places = list(self.place.bees)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            p = self.place.exit
            while p is not None:
                if p.ant is not None and p.ant not in self.buffed:
                    p.ant.damage = 2* p.ant.damage
                    self.buffed+=[p.ant]
                    if hasattr(p.ant, 'ant') and p.ant.ant is not None and p.ant.ant not in self.buffed:
                        p.ant.ant.damage = 2*p.ant.ant.damage
                        self.buffed+=[p.ant.ant]

                p = p.exit
            colony.queen = QueenPlace(self.place, colony.queen)


            #check if the inside is an instance of a queen ant 
            #suggestion: print check 
            #everytime move to the next exit increment a variable keeps track o
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0 and self.imposter == False:
            return bees_win()
        elif self.armor == 0 and self.imposter == True:
            self.place.remove_insect(self)
